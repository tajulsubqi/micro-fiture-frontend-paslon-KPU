...
const Chart: React.FC<Chart> = () => {
  ...
  return (
    <div className="Container my-5 mx-auto" style={{ width: 1140 }}>
      <VoteModal
        show={showVoteModal}
        onHide={handleCloseVoteModal}
        handleVote={handleVote}
      />
      <h1
        className="text-center fw-bold mb-5"
        style={{ color: '#5E5A00', marginTop: '7rem' }}
      >
        ...
        <div className="d-flex flex-column justify-content-center align-content-center mt-5 w-50 fw-bold">
          {sortedCandidates.map((item, index) => (
            <Card
              key={index}
              body
              className={`mt-3 ${item.color} text-white text-center fs-4`}
            >
              {item.candidate}
            </Card>
          ))}
      ...
      <div className="d-flex justify-content-center mt-5">
        <button
          ...
          className="btn text-white fw-bold fs-5 px-4"
